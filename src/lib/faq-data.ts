...
export const FAQ_CATEGORIES: FaqCategory[] = [
  ...
  },
  {
    ...
    items: [
      {
        q: "Does it actually answer the phone, or just text back?",
        a: "Both, depending on the setup you choose. The AI receptionist can answer live calls and hold a conversation with the caller. If a call still goes unanswered, the missed call text-back runs as the backstop. You can start with text-back alone and add live answering later.",
      },
      {
        q: "Will callers know they're talking to AI?",
        a: "Yes. The receptionist identifies itself as an automated assistant at the start of the call. We don't configure systems that pretend to be a specific person on your staff, and disclosure requirements in some states make this the right default regardless.",
      },
      {
...
// Curated set of the highest-intent questions shown on the homepage, pulled
// from the categories above so the answers can't drift out of sync with the
// full /faq page. Exported so the homepage can emit FAQPage structured data
// for exactly the questions it renders.
const HOME_FAQ_QUESTIONS = [
  "Does it actually answer the phone, or just text back?",
...
export const HOME_FAQS: FaqItem[] = HOME_FAQ_QUESTIONS.map(
  (q) => ALL_FAQ_ITEMS.find((item) => item.q === q)!
).filter(Boolean);

/**
 * Questions attached to each service page.
 *
 * Every entry is an existing, approved answer from the categories above —
 * nothing new is asserted here. Questions are assigned to as few services as
 * possible so the same Q&A pair isn't marked up as FAQPage on several URLs at
 * once; the pricing and contract questions stay on /faq only.
 */
const SERVICE_FAQ_QUESTIONS: Record<string, string[]> = {
  "ai-receptionist": [
    "Does it actually answer the phone, or just text back?",
    "Will callers know they're talking to AI?",
    "Does the AI actually understand what customers are asking?",
    "What happens after hours or on weekends?",
  ],
  "missed-call-recovery": [
    "What happens when I miss a call?",
    "Will this work with my current phone number?",
    "What happens if I don't use something like this?",
    "What phone systems does this work with?",
  ],
  "automated-appointment-booking": [
    "Can the system book appointments on its own?",
    "Can it connect with my calendar?",
    ...
    "Can I control which time slots are available?",
  ],
  "intelligent-lead-capture": [
    "Can it capture leads from my website forms?",
    "Does it integrate with my existing CRM or calendar?",
    "Is my customer data secure?",
    "What happens to my data if I ever cancel?",
    "What do you need from me to get started?",
  ],
  "custom-workflow-automation": [
    "Can I customize the messages?",
    "How do I make changes once it's live?",
    "Can it support multiple locations?",
    "Do I need to be tech-savvy?",
    "What if something breaks?",
  ],
  "omnichannel-communication": [
    "What if a customer prefers to talk instead of text?",
...
export function faqsForService(slug: string): FaqItem[] {
  const questions = SERVICE_FAQ_QUESTIONS[slug];
  if (!questions) return [];
  return questions
    .map((q) => ALL_FAQ_ITEMS.find((item) => item.q === q))
